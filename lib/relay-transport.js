...
export class RelayTransport {
  // botName → { relay: {resolve, timer, requestId, payload}|null, poll: {resolve, timer}|null, queued: payload|null }
  //
  ...
  // queued is exclusively for inject() — kick poison pills that have no relay promise.
  #bots    = new Map();
  #counter = 0;
  ...
  #getBot(botName) {
    ...
    if (!state) {
      state = { relay: null, poll: null, queued: null };
  ...
  poll(botName, timeoutMs = 120_000) {
    ...
    // Injected payload (kick) takes priority
    if (bot.queued) {
      const payload = bot.queued;
      bot.queued = null;
   ...
   * Used to deliver kick poison pills. Bypasses the relay slot entirely.
   */
