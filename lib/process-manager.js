...
export class ProcessManager {
  ...
  start() {
    ...
    console.log('[hub] Starting world server...');
    ...
    this.#child.on('exit', (code, signal) => {
      ...
      if (this.#stopped || code === 0 || signal === 'SIGTERM') {
        console.log('[hub] World server exited cleanly');
        return;
      }
      ...
      console.error(
        `[hub] World server crashed (code=${code}, signal=${signal}), ` +
    ...
    this.#child.on('error', (err) => {
      console.error(`[hub] World server spawn error: ${err.message}`);
    });
  }
